;(function() {
    ...
    function getAliases(index) {
        ...
        return index !== null && this._aliases
            ? this._aliases[index]
            : this._aliases;
    }
...
//        if (result) {
//            var t1 = result.split('.').pop();
//
//            result = _.trim(_.trim(result.split('.').pop()), "'").split('var ').pop();
//        }

        var result = (this.entry.match(/\*\/\s*(?:function ([^(]*)|(.*?)(?=[:=,]|return\b))/) || []).pop();
        if (result) {
            var resultArray = result.split('.');
            var str = resultArray.pop() || '';
            var trimmed = str.replace(/(^'+)|('+$)/,'').trim();
            var resultArray = trimmed.split('var ');
    ...
    function getDesc() {
        ...
        if (result) {
            ...
            result = (type == 'Function' ? '' : '(' + type.replace(/(^\{)|(\}$)/,'').replace(/\|/g, ', ') + '): ') + result;
        }
        this._desc = result;
        return result;
    }
    ...
    function isPrivate() {
        if (this._isPrivate == null) {
            this._isPrivate = (
                this.isLicense() ||
                    hasTag(this.entry, 'private') || !hasTag(this.entry, '*')
                );
        }
    ...
    function getParams(index) {
        if (this._params == null) {
            var tuplesRaw = this.entry.match(/^ *\*[\t ]*@param\s+\{\(?([^})]+)\)?\}\s+(\[.+\]|[\w|]+(?:\[.+\])?)\s+([\s\S]*?)(?=\*\s\@[a-z]|\*\/)/gm) || [];
            var tuples = _.compact(tuplesRaw.slice(1)),
                result = [];
     ...
     */
    module.exports.getEntries = function getEntries(source) {
