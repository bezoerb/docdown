(function() {
     ...
     * @param {Array} _options The options array.
     */
    function MarkdownGenerator(_source, _options) {
        var options,source,entries;
        ...
        if (_.isObject(_source)) {
            ...
            _options['source'] = _source;
        }

        try {
            source = _options.hasOwnProperty('source') && _options['source'];
            source = Path.resolve(source);
            _options.path = FS.realpathSync(_options['source']);
        } catch (err) {
            // _source is no real path
        }

        if (_options.path) {
            _options.source = FS.readFileSync(_options.path, 'UTF-8');

            var ext = Path.extname(_options.path).substring(1);
            if (!_options.lang && ext) {
                _options.lang = ext;
            }

            if (!_options.title) {
                _options.title = Path.basename(_options.path) + ' API documentation';
            }
        }
        ...
        if (!_options.toc) {
            _options.toc = 'properties';
        }

        options = _options;
        source = _options.source.replace(/[\r\n]*/, "\n");
        entries = Entry.getEntries(source);

        _.forEach(entries, function(value, index) {
            entries[index] = new Entry(value, source, options.lang);
        }, this);
        ...
        function format($string) {
            ...
            _.forEach(tokenized, function(snippet, index) {
                $string = $string.replace('__token' + index + '__', snippet);
            });
...
//            str = str.replace(/@@token@@/, swapTokensToSnippets);
        ...
        function getHash(entry, member) {
            entry = _.isNumber(entry) ? entries[entry] : entry;
        ...
        function getLineUrl(entry) {
            entry = _.isNumber(entry) ? entries(entry) : entry;
            return options['url'] + '#L' + entry.getLineNumber();
        }
        ...
        function generate() {
            var $api = {},
                $byCategory = options['toc'] === 'categories',
                $categories = {},
                $closeTag = closeTag,
                $compiling = false,
                $openTag = openTag,
            ...
            _.forEach($api, function(entry, index) {
                entry.hash = getHash(entry);
                entry.href = getLineUrl(entry);
                entry.separator = '';

                var member = entry.getMembers(0);
                member = (member ? member + getSeparator(entry) : '') + entry.getName();
                ...
                _.forEach(['static', 'plugin'], function(kind) {
                    _.forEach(entry[kind], function(subentry) {
                        subentry.hash = getHash(subentry);
                        subentry.href = getLineUrl(subentry);
                        subentry.member = member;
                        subentry.separator = getSeparator(subentry);
                    });
                });
...
//        uksort($api, 'sortCompare');
            ...
            _.forEach($api, function(entry) {
                ...
                _.forEach(['static', 'plugin'], function(kind) {
                    _.forEach(entry[kind], function(subentry) {
                        if (!$categories.hasOwnProperty(subentry.getCategory())) {
